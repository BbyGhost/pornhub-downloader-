...
internal static class Program
{
    ...
    public static void Main()
    {
        try
        {
            ...
            while (true)
            {
                ...
                if (action == "probe") Probe(url,referer,origin,ua);
                else if (action == "download") Download(url,Get(root,"filename"),referer,origin,ua);
            }
        }
    ...
    static void AddHeaders(ProcessStartInfo psi,string referer,string origin,string ua)
    {
    ...
    static void Probe(string url,string referer,string origin,string ua)
    {
        try
        {
            ...
            psi.ArgumentList.Add("-hide_banner"); AddHeaders(psi,referer,origin,ua); psi.ArgumentList.Add("-i");psi.ArgumentList.Add(url); psi.ArgumentList.Add("-map");psi.ArgumentList.Add("0:v?"); psi.ArgumentList.Add("-f");psi.ArgumentList.Add("null");psi.ArgumentList.Add("-");
    ...
    static void Download(string url,string filename,string referer,string origin,string ua)
    {
        try
        {
            ...
            var psi=new ProcessStartInfo{FileName=Ffmpeg(),UseShellExecute=false,RedirectStandardError=true,RedirectStandardOutput=true,CreateNoWindow=true};
            psi.ArgumentList.Add("-hide_banner");psi.ArgumentList.Add("-y");AddHeaders(psi,referer,origin,ua);psi.ArgumentList.Add("-i");psi.ArgumentList.Add(url);psi.ArgumentList.Add("-map");psi.ArgumentList.Add("0:v:0?");psi.ArgumentList.Add("-map");psi.ArgumentList.Add("0:a:0?");psi.ArgumentList.Add("-c");psi.ArgumentList.Add("copy");psi.ArgumentList.Add("-movflags");psi.ArgumentList.Add("+faststart");psi.ArgumentList.Add(output);
            ...
            p.WaitForExit(); if(p.ExitCode!=0)throw new Exception("FFmpeg failed: "+last); if(!File.Exists(output)||new FileInfo(output).Length==0)throw new Exception("Output file was not created."); Send(new {@event="complete",path=output,progress=100});
        }
        catch(Exception ex){Log(ex);Send(new {@event="error",error=ex.Message});}
    }
